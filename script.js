...
const messages = {
  cho_atari: [
    ...
    { main: "やったねー！", sub: "幸運運ぶ、えんじぇるななか" },
    { main: "神引きだ！", sub: "みんなを照らす、シャイニーななか" },
    { main: "おめでとう！", sub: "今日も全開、パーフェクトななか" },
    { main: "超はっぴー！", sub: "運命感じる、ロマンチックななか" }
  ],
  ooatari: [
    { main: "大当たり！", sub: "今日も一緒に、スペシャルななか" },
    { main: "大勝利！", sub: "今日も明日も、ずっと無敵ななか" },
    { main: "イエイイエイ！", sub: "ルンルン気分の、ハッピーななか" },
    { main: "さすがだね！", sub: "頼りになるよ、ジーニアスななか" },
    { main: "いいかんじ！", sub: "笑顔がキラリ、アイドルななか" },
    { main: "ツイてるね！", sub: "ご機嫌ナナメな、わけがないななか" },
    { main: "わぁーい！", sub: "お祝いしよっか、フェスティバルななか" }
  ],
  atari: [
    { main: "やったじゃん！", sub: "今日も幸運、スーパーななか" },
    { main: "よかったね！", sub: "のんびりいこう、マイペースななか" },
    { main: "ナイスだよ！", sub: "いつも元気に、スマイルななか" },
    { main: "いいじゃんね！", sub: "今日はまったり、リラックスななか" },
    { main: "やったあー！", sub: "おやつを食べよう、食いしん坊ななか" },
    { main: "フフフのフ！", sub: "ちょっとドヤ顔、得意げななか" },
    { main: "ラッキーだ！", sub: "こっそり応援、チアガールななか" }
  ],
  hazure: [
    { main: "はずれてない！笑", sub: "今日も一緒に、大好きななか" },
    { main: "大丈夫！", sub: "わたしがついてる、とってもななか" },
    { main: "どんまいっ！", sub: "明日があるさ、ポジティブななか" },
    { main: "およよよよ？", sub: "ちょっぴりドジっ子、てへぺろななか" },
    { main: "気にしない！", sub: "一緒に笑おう、スマイリーななか" },
    { main: "あれれれれ？", sub: "迷子になっても、たくましいななか" },
    { main: "ざんねーん！", sub: "でもでも心は、大吉ななか" }
  ],
  dai_hazure: [
    { main: "任せとけ！", sub: "今日もあたしは、ポンコツななか" },
    { main: "大はずれ！？", sub: "むしろおいしい、芸人ななか" },
    { main: "うっそーん！", sub: "涙はポイッだ、強がりななか" },
    { main: "ぴえんだよ…", sub: "ヨシヨシしてね、甘えん坊ななか" },
    { main: "マジですか…", sub: "逆に運命、ミラクルななか" },
    { main: "大ピンチ！？", sub: "ここから逆転、ヒーローななか" },
    { main: "やっちまった！", sub: "これもご愛嬌、キュートななか" }
  ]
};

// 【完全修正版】絵の通りに「全12本」の線を正確に記憶させました！
const bridges = [
  { y: 0.27, line: 3 }, // ① 黄色と紫の間（一番上）
  { y: 0.28, line: 0 }, // ② ピンクと薄ピンクの間（上）
  { y: 0.35, line: 2 }, // ③ オレンジと黄色の間（上）
  { y: 0.39, line: 3 }, // ④ 黄色と紫の間（上から2番目）
  { y: 0.41, line: 1 }, // ⑤ 薄ピンクとオレンジの間（上）
  { y: 0.48, line: 2 }, // ⑥ オレンジと黄色の間（真ん中）
  { y: 0.51, line: 0 }, // ⑦ ピンクと薄ピンクの間（下）
  { y: 0.52, line: 3 }, // ⑧ 黄色と紫の間（上から3番目）
  { y: 0.55, line: 1 }, // ⑨ 薄ピンクとオレンジの間（真ん中・グレーの線）
  { y: 0.59, line: 2 }, // ⑩ オレンジと黄色の間（下・グレーの線）
  { y: 0.62, line: 1 }, // ⑪ 薄ピンクとオレンジの間（下・ピンクの線）
  { y: 0.67, line: 3 }  // ⑫ 黄色と紫の間（一番下）
].sort((a, b) => a.y - b.y);
...
function initGame() {
  ...
  currentGoals.forEach((goal, index) => {
    ...
    goalNode.className = "goal-text-box"; 
...
async function startAmidaAnimation(startIndex) {
  ...
  const resultFruit = headPhoneFruit;
  ...
  if (!runner) {
    ...
    linesWrapper.appendChild(runner);
  }
  ...
  const startX = lineXPositions[path[0].line];
  const startY = wrapperRect.height * path[0].y - 22;
  ...
  setTimeout(() => {
    ...
    showResultScreen(resultFruit, resultCategory);
  }, 300);
}
